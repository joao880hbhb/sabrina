...
// Helper to manage storage version
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const deleteOrder = (id: string) => {
    ...
    if (targetOrder.orderStatus !== "returned") {
    ...
    if (targetOrder.payoutStatus === "settled") {
      const relatedCashTx = cashTransactions.find(
        (tx) => tx.referenceId === targetOrder.id,
      );
      if (relatedCashTx) {
        setAccounts((prev) =>
          prev.map((acc) =>
            acc.id === relatedCashTx.accountId
              ? {
                  ...acc,
                  balance: Math.max(0, acc.balance - targetOrder.netPayout),
                }
              : acc,
          ),
        );
    ...
    setCashTransactions((prev) =>
      prev.filter((tx) => tx.referenceId !== targetOrder.id),
    );
    setInventoryTransactions((prev) =>
      prev.filter((tx) => tx.referenceNo !== targetOrder.orderNumber),
    );
  };
