...
interface AppContextType {
  ...
  loginWithEmailPassword: (email: string, password: string) => UserAccount | null;
  ...
  updateOrderTracking: (orderId: string, resiStatus: ResiDeliveryStatus, checkpoint?: TrackingCheckpoint) => void;
  processPayoutSettlement: (orderId: string, accountId: string) => void;
  processOrderReturn: (orderId: string, reason: string, restockToWarehouse: boolean) => void;
  ...
  updateMaterialStock: (id: string, changeQty: number, avgCost?: number) => void;
  recordInventoryTransaction: (tx: Omit<InventoryTransaction, 'id' | 'transactionNumber'>) => void;
  ...
  recordStockMutation: (skuId: string, qty: number, from: string, to: string, notes: string) => void;
  createStockOpname: (opname: Omit<StockOpnameRecord, 'id' | 'opnameNumber'>) => void;
  approveStockOpname: (id: string) => void;

  // Production & SPK
  productionPlans: ProductionPlan[];
  createProductionSPK: (plan: Omit<ProductionPlan, 'id' | 'spkNumber' | 'createdAt'>) => void;
  updateProductionStatus: (id: string, status: ProductionPlan['status'], finishedQty?: number, rejectQty?: number, reworkQty?: number, actualHpp?: number) => void;

  // Finance
  accounts: FinancialAccount[];
  cashTransactions: CashTransaction[];
  debts: DebtPayable[];
  addCashTransaction: (tx: Omit<CashTransaction, 'id' | 'transactionNumber'>) => void;
  ...
  createSuratJalan: (sj: Omit<SuratJalan, 'id' | 'nomorSuratJalan'>) => SuratJalan;
  updateSuratJalanStatus: (id: string, status: SuratJalan['status']) => void;
  updateSuratJalanSignatures: (id: string, pengirimSig?: string, kurirSig?: string, kurirName?: string, platNomor?: string, kurirPhone?: string) => void;
  updatePackageScanStatus: (suratJalanId: string, resiNumber: string, newStatus: 'scanned' | 'pending_scan' | 'missing_alert') => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [inventoryTransactions, setInventoryTransactions] = useState<InventoryTransaction[]>(() => {
    try {
      ...
      console.warn('Failed to parse inventoryTransactions from localStorage', e);
    }
  ...
  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY_PREFIX}inv_tx`, JSON.stringify(inventoryTransactions));
  }, [inventoryTransactions]);

  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY_PREFIX}stock_opnames`, JSON.stringify(stockOpnames));
  }, [stockOpnames]);

  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY_PREFIX}production`, JSON.stringify(productionPlans));
  }, [productionPlans]);
  ...
  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY_PREFIX}cash_tx`, JSON.stringify(cashTransactions));
  }, [cashTransactions]);
  ...
  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY_PREFIX}surat_jalan`, JSON.stringify(suratJalanList));
  }, [suratJalanList]);
  ...
  const loginWithEmailPassword = (email: string, password: string) => {
    ...
    const matchedUser = users.find(user => {
      ...
      return user.email.trim().toLowerCase() === normalizedEmail && savedPassword === password;
    });
  ...
  const updateUserSettings = (userId: string, settingsUpdate: Partial<UserSettings>) => {
  ...
  const addProduct = (prodData: Omit<ProductSKU, 'id' | 'createdAt'>) => {
    ...
    const marginPercent = prodData.sellingPrice > 0 ? (marginRp / prodData.sellingPrice) * 100 : 0;
  ...
  const updateProduct = (id: string, updateData: Partial<ProductSKU>) => {
    setProducts(prev => prev.map(item => {
      ...
      if (updateData.hppBreakdown || updateData.sellingPrice !== undefined) {
        ...
        merged.marginPercent = merged.sellingPrice > 0 ? (merged.marginRp / merged.sellingPrice) * 100 : 0;
      }
      return merged;
    }));
  };
  ...
  const updateProductMinStock = (id: string, minStockAlert: number) => {
    setProducts(prev => prev.map(p => p.id === id ? { ...p, minStockAlert: Math.max(0, minStockAlert) } : p));
  };
  ...
  const updateOrderTracking = (orderId: string, resiStatus: ResiDeliveryStatus, checkpoint?: TrackingCheckpoint) => {
    ...
    setOrders(prev => prev.map(o => {
      ...
      } else if ((resiStatus === 'in_transit' || resiStatus === 'out_for_delivery' || resiStatus === 'picked_up') && o.orderStatus === 'processing') {
  ...
  const processPayoutSettlement = (orderId: string, accountId: string) => {
    ...
    setAccounts(prev => prev.map(acc => acc.id === targetAccount.id ? {
      ...acc,
  ...
  const processOrderReturn = (orderId: string, reason: string, restockToWarehouse: boolean) => {
    const order = orders.find(o => o.id === orderId);
    if (!order) return;
  ...
  const importBatchOrders = (importedOrders: Omit<MarketplaceOrder, 'id'>[]) => {
  ...
  const updateMaterialStock = (id: string, changeQty: number, avgCost?: number) => {
  ...
  const recordInventoryTransaction = (txData: Omit<InventoryTransaction, 'id' | 'transactionNumber'>) => {
  ...
  const recordStockMutation = (skuId: string, qty: number, from: string, to: string, notes: string) => {
    const product = products.find(p => p.id === skuId);
    if (!product) return;

    if (from.toLowerCase().includes('gudang') && to.toLowerCase().includes('marketplace')) {
      setProducts(prev => prev.map(p => p.id === skuId ? {
        ...p,
        stockGudang: Math.max(0, p.stockGudang - qty),
        stockMarketplace: p.stockMarketplace + qty
      } : p));
    } else if (from.toLowerCase().includes('marketplace') && to.toLowerCase().includes('gudang')) {
      setProducts(prev => prev.map(p => p.id === skuId ? {
        ...p,
  ...
  const createStockOpname = (opnameData: Omit<StockOpnameRecord, 'id' | 'opnameNumber'>) => {
  ...
  const approveStockOpname = (id: string) => {
    ...
    setProducts(prev => prev.map(p => p.id === record.skuId ? {
      ...p,
      stockGudang: record.physicalStock
    } : p));

    setStockOpnames(prev => prev.map(r => r.id === id ? { ...r, status: 'approved' } : r));
  ...
  const processManualInventoryMovement = (movement: {
    ...
    const prod = isProduct ? products.find(p => p.id === movement.skuId) : undefined;
    const mat = !isProduct && movement.materialId ? materials.find(m => m.id === movement.materialId) : undefined;
    ...
    if (isProduct && prod) {
      unit = 'pcs';
      if (movement.type === 'BARANG_MASUK_PRODUKSI' || movement.type === 'BARANG_MASUK_PEMBELIAN' || movement.type === 'RETUR_MASUK') {
        qtyChange = Math.abs(movement.qty);
        setProducts(prev => prev.map(p => p.id === prod.id ? { ...p, stockGudang: p.stockGudang + qtyChange } : p));
      } else if (movement.type === 'BARANG_KELUAR_PENJUALAN' || movement.type === 'RETUR_REJECT') {
        qtyChange = -Math.abs(movement.qty);
        setProducts(prev => prev.map(p => p.id === prod.id ? { ...p, stockGudang: Math.max(0, p.stockGudang + qtyChange) } : p));
      } else if (movement.type === 'MUTASI_GUDANG') {
        recordStockMutation(prod.id, movement.qty, movement.fromLocation, movement.toLocation, movement.notes || '');
        return;
      }
    } else if (mat) {
      unit = mat.unit;
      if (movement.type === 'PEMBELIAN_BAHAN' || movement.type === 'BARANG_MASUK_PEMBELIAN') {
        qtyChange = Math.abs(movement.qty);
    ...
    const txPrefix = movement.type.startsWith('BARANG_MASUK') || movement.type === 'PEMBELIAN_BAHAN' ? 'IN' : 
                     movement.type === 'RETUR_MASUK' ? 'RET' : 'OUT';
    ...
    const tx: InventoryTransaction = {
      ...
      date: new Date().toISOString().split('T')[0],
      type: movement.type,
      referenceNo: movement.referenceNo || `MANUAL-${Date.now().toString().slice(-4)}`,
      skuId: prod?.id,
      skuCode: prod?.sku,
      ...
      pic: currentUser.name,
      notes: movement.notes || `Pencatatan pergerakan stok manual (${movement.type})`
    };
  ...
  // Production & SPK
  const createProductionSPK = (planData: Omit<ProductionPlan, 'id' | 'spkNumber' | 'createdAt'>) => {
    ...
    planData.materialUsed.forEach(mat => {
      setMaterials(prev => prev.map(m => m.id === mat.materialId ? {
        ...m,
  ...
  const updateProductionStatus = (
    ...
    const finalFinished = finishedQty !== undefined ? finishedQty : plan.finishedGoodQty;
    ...
    if (status === 'completed' && plan.status !== 'completed' && finalFinished > 0) {
      setProducts(prev => prev.map(prod => prod.id === plan.skuId ? {
        ...prod,
  ...
  // Finance
  const addCashTransaction = (txData: Omit<CashTransaction, 'id' | 'transactionNumber'>) => {
    ...
    setAccounts(prev => prev.map(acc => {
      if (acc.id === txData.accountId) {
        return {
          ...acc,
          balance: isIncome ? acc.balance + txData.amount : acc.balance - txData.amount
        };
      }
      return acc;
    }));
  ...
  const payDebt = (id: string, amount: number, accountId: string) => {
    ...
    const cashTx: CashTransaction = {
      id: `ctx-${Date.now()}`,
      transactionNumber: isHutang ? `KK-${Date.now().toString().slice(-6)}` : `KM-${Date.now().toString().slice(-6)}`,
      date: new Date().toISOString().split('T')[0],
  ...
  // Surat Jalan
  const createSuratJalan = (sjData: Omit<SuratJalan, 'id' | 'nomorSuratJalan'>): SuratJalan => {
  ...
  const updateSuratJalanStatus = (id: string, status: SuratJalan['status']) => {
    setSuratJalanList(prev => prev.map(s => s.id === id ? { ...s, status } : s));
  };
  ...
  const createPackageDeliverySuratJalan = (data: {
    ...
    const count = suratJalanList.filter(s => s.tipeSuratJalan === 'pengantaran_paket_marketplace').length + 1;
    const yearMonth = new Date().toISOString().slice(0, 10).replace(/-/g, '');
    const ekspPrefix = data.ekspedisi.includes('Shopee') ? 'SPX' :
                       data.ekspedisi.includes('J&T') ? 'JNT' :
                       data.ekspedisi.includes('SiCepat') ? 'SCP' :
                       data.ekspedisi.includes('JNE') ? 'JNE' : 'EXP';
    ...
    const newSJ: SuratJalan = {
      ...
      catatan: data.catatan || `Manifest serah terima ${data.packages.length} paket ke kurir ${data.ekspedisi}. Bukti serah terima sah.`,
      status: 'dikirim'
    };
    ...
    setOrders(prev => prev.map(o => {
      if (orderIds.includes(o.id)) {
        ...
        return {
          ...
          resiStatus: (o.resiStatus === 'pending_pickup' || !o.resiStatus) ? 'picked_up' : o.resiStatus,
  ...
  const updateSuratJalanSignatures = (
    ...
    setSuratJalanList(prev => prev.map(s => {
      if (s.id !== id) return s;
      return {
        ...s,
        pengirim: {
          ...s.pengirim,
          signatureDataUrl: pengirimSig !== undefined ? pengirimSig : s.pengirim.signatureDataUrl,
          signatureDate: pengirimSig ? `${dateStr} ${timeStr}` : s.pengirim.signatureDate
        },
        penerima: {
          ...s.penerima,
          nama: kurirName || s.penerima.nama,
          signatureDataUrl: kurirSig !== undefined ? kurirSig : s.penerima.signatureDataUrl,
          signatureDate: kurirSig ? `${dateStr} ${timeStr}` : s.penerima.signatureDate
        },
  ...
  const updatePackageScanStatus = (
        ...
        packages: s.packages.map(p => {
          if (p.resiNumber === resiNumber) {
            return {
              ...
              scannedAt: newStatus === 'scanned' ? `${dateStr} ${timeStr}` : p.scannedAt
            };
          }
          return p;
        })
    ...
    setOrders(prev => prev.map(o => {
      if (o.resiNumber === resiNumber) {
        ...
        } else if (newStatus === 'scanned' && o.resiStatus === 'lost_or_unscanned') {
          return {
            ...o,
