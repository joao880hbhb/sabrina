...
export const MarketplaceSales: React.FC = () => {
  ...
  // Primary View Mode
  const [viewMode, setViewMode] = useState<"pesanan" | "tracing" | "payout">(
    "pesanan",
  );
  ...
  const [showReturnModal, setShowReturnModal] =
    useState<MarketplaceOrder | null>(null);
  const [trackingOrder, setTrackingOrder] = useState<MarketplaceOrder | null>(
    null,
  );
  const [selectedAccountForSettlement, setSelectedAccountForSettlement] =
  ...
  const filteredOrders = orders.filter((o) => {
    const matchChannel =
      selectedChannel === "all" || o.channel === selectedChannel;
    ...
    const matchPayout =
      payoutFilter === "all" || o.payoutStatus === payoutFilter;
    const matchResi =
      resiStatusFilter === "all" || o.resiStatus === resiStatusFilter;

    return (
      matchChannel && matchSearch && matchStatus && matchPayout && matchResi
    );
  });
  ...
  const totalVoucherDiscount = filteredOrders.reduce(
    (s, o) => s + o.voucherAmount + o.discountAmount + o.shippingSubsidy,
    0,
  );
  const totalNet = filteredOrders.reduce((s, o) => s + o.netPayout, 0);
  const totalEscrow = filteredOrders
    .filter((o) => o.payoutStatus === "escrow")
    .reduce((s, o) => s + o.netPayout, 0);
  ...
  const pendingPickupCount = orders.filter(
    (o) => o.resiStatus === "pending_pickup",
  ).length;
  const inTransitCount = orders.filter(
    (o) =>
      o.resiStatus === "in_transit" ||
      o.resiStatus === "out_for_delivery" ||
      o.resiStatus === "picked_up",
  ).length;
  const deliveredCount = orders.filter(
    (o) => o.resiStatus === "delivered",
  ).length;
  const lostOrUnscannedCount = orders.filter(
    (o) => o.resiStatus === "lost_or_unscanned",
  ).length;
  ...
  const handleAddItemToOrder = (productId: string) => {
    ...
    const feeRate =
      newOrder.channel === "Shopee"
        ? 0.085
        : newOrder.channel === "TikTok Shop"
          ? 0.08
          : newOrder.channel === "Tokopedia"
            ? 0.065
            : newOrder.channel === "Lazada"
              ? 0.07
              : 0.05;
    ...
    const net =
      gross -
      (calculatedAdmin +
        newOrder.voucherAmount +
        newOrder.discountAmount +
        newOrder.shippingSubsidy);
  ...
  const handleCreateOrderSubmit = (e: React.FormEvent) => {
    ...
    const resiNum =
      newOrder.resiNumber || `SPXID${Date.now().toString().slice(-8)}`;
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const parsedOrders: Omit<MarketplaceOrder, "id">[] = data.map(
          (row, idx) => {
            const channelName = (row.Marketplace ||
              row.Channel ||
              row.channel ||
              "Shopee") as MarketplaceChannel;
            const gross = Number(
              row["Harga Total"] ||
                row.Gross ||
                row.grossAmount ||
            ...
            const voucher = Number(
              row["Voucher Toko"] || row.Voucher || row.voucherAmount || 0,
            );
            ...
            const resi = String(
              row["No Resi"] ||
                row.Resi ||
                row.resiNumber ||
                `IMP-RESI-${Date.now()}-${idx}`,
            );

            // Match or fallback product
            const skuCode =
              row.SKU || row.sku || products[0]?.sku || "SBH-GMS-SLK-SGE-M";
            const matchedProd =
              products.find((p) => p.sku === skuCode) || products[0];
            ...
            return {
              ...
              customerName: String(
                row.Pembeli || row.Customer || "Customer Marketplace",
              ),
              ...
              trackingHistory: [
                {
                  ...
                  description:
                    "Paket telah lolos sortir dan diberangkatkan ke gateway tujuan.",
  ...
  const exportOrdersToExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `Laporan_Penjualan_Marketplace_${selectedChannel}_${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  };
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-xs">
        <div>
          <div className="flex items-center gap-2">
            <span className="p-1 rounded-md bg-blue-100 text-blue-800 text-xs font-bold uppercase">
              Marketplace Online
            </span>
          ...
          <p className="text-xs sm:text-sm text-slate-600 mt-1">
            Manajemen order Shopee, TikTok Shop, Tokopedia, tracing resi
            real-time, audit serah terima kurir, dan pencairan escrow.
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
        ...
        >
          ...
          <span>
            Rekapitulasi Payout Escrow (
            {filteredOrders.filter((o) => o.payoutStatus === "escrow").length})
          </span>
        </button>
      </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <div
              ...
              className={`p-4 bg-white rounded-2xl border transition-all cursor-pointer shadow-xs ${
                resiStatusFilter === "all"
                  ? "border-indigo-600 ring-2 ring-indigo-200"
                  : "border-slate-200 hover:border-slate-300"
              }`}
            >
              <span className="text-[10px] font-bold uppercase text-slate-400">
                Total Resi Marketplace
              </span>
              <p className="text-2xl font-black text-slate-900 mt-1">
                {totalActiveResi}
              </p>
              <span className="text-[11px] text-slate-500">
                Semua order dengan AWB
              </span>
            </div>
            ...
            <div
              ...
              className={`p-4 bg-amber-50/70 rounded-2xl border transition-all cursor-pointer shadow-xs ${
                resiStatusFilter === "pending_pickup"
            ...
            >
              ...
              <p className="text-2xl font-black text-rose-600 mt-1">
                {lostOrUnscannedCount}
              </p>
              <span className="text-[11px] text-rose-700 font-semibold">
                {lostOrUnscannedCount > 0
                  ? "Klaim Siap dengan TTD Kurir"
                  : "Nol Resi Bermasalah"}
              </span>
            </div>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-bold text-rose-950 text-sm">
                    Peringatan Resi Belum Ter-scan Ekspedisi (
                    {lostOrUnscannedCount} Paket Terdeteksi)
                  </h4>
                  <p className="text-xs text-rose-800 mt-0.5 leading-relaxed">
                    Paket fisik sudah diserahkan ke kurir tetapi belum ter-scan
                    inbound di gateway logistik &gt; 12 jam. Gunakan nomor Surat
                    Jalan dan{" "}
                    <strong>Tanda Tangan Digital Kurir Penjemput</strong>{" "}
                    sebagai bukti sah hukum untuk klaim ganti rugi 100%.
                  </p>
                </div>
              </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end">
              ...
              >
                ...
                <option value="lost_or_unscanned">
                  🚨 Belum Terscan / Hilang
                </option>
              </select>
                              ...
                              {order.resiNumber && (
                                <button
                                  onClick={() => {
                                    ...
                                    alert(
                                      `Nomor resi ${order.resiNumber} disalin!`,
                                    );
                                  }}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
            {channels.map((ch) => {
              ...
              const count =
                ch.key === "all"
                  ? orders.length
                  : orders.filter((o) => o.channel === ch.key).length;
              return (
                <button
                  key={ch.key}
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <span
                      className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${ch.iconBg}`}
                    >
                      {ch.name.slice(0, 3)}
                    </span>
                    <span
                      className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${isSelected ? "bg-slate-800 text-indigo-400" : "bg-slate-100 text-slate-600"}`}
                    >
                      {count}
                    </span>
                  </div>
                  <div>
                    ...
                    <p
                      className={`text-[10px] ${isSelected ? "text-slate-300" : "text-slate-400"}`}
                    >
                      {ch.feeRate}
                    </p>
                  </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
            ...
            <div className="p-3 bg-amber-50 rounded-xl border border-amber-100">
              <span className="text-[10px] font-bold uppercase text-amber-700">
                Voucher, Diskon & Ongkir
              </span>
              <p className="text-base sm:text-lg font-black text-amber-800 mt-0.5">
                - Rp {totalVoucherDiscount.toLocaleString("id-ID")}
              </p>
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
                  <tr>
                    ...
                    <th className="py-3 px-3 text-right font-bold text-emerald-700">
                      Net Cair
                    </th>
                  ...
                  {filteredOrders.length === 0 ? (
                    <tr>
                      ...
                      >
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">
                          Coba sesuaikan filter atau input pesanan baru.
                        </p>
                      </td>
                    </tr>
                  ) : (
                    filteredOrders.map((order) => (
                      ...
                      >
                        ...
                        <td className="py-3.5 px-3 text-right font-mono text-rose-600">
                          {order.adminFee > 0
                            ? `- Rp ${order.adminFee.toLocaleString("id-ID")}`
                            : "-"}
                        </td>
                          ...
                          <div className="flex items-center justify-center gap-1.5 flex-wrap">
                            ...
                            <button
                              onClick={() => {
                                if (
                                  confirm(
                                    `Yakin ingin menghapus transaksi ${order.orderNumber}?`,
                                  )
                                ) {
                                  deleteOrder(order.id);
                                }
                              }}
                              className="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-lg transition-colors shadow-2xs"
          ...
          <div className="p-4 bg-white rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-base font-bold text-slate-900">
                Rekapitulasi Saldo Escrow Belum Cair
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Total dana penjualan yang sedang ditahan oleh sistem marketplace
                sampai paket berstatus diterima pembeli.
              </p>
            </div>
            <div className="text-right">
              <span className="text-xs font-semibold text-slate-400">
                Total Escrow Menunggu Pencairan:
              </span>
            ...
            <form onSubmit={handleCreateOrderSubmit} className="mt-4 space-y-4">
                          ...
                          : ch === "TikTok Shop"
                            ...
                            : ch === "Tokopedia"
                              ...
                              : ch === "Lazada"
                                ? 0.07
                                : 0.05;
                      const calculatedAdmin = Math.round(
                        newOrder.grossAmount * feeRate,
                      );
                      const net =
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewOrder({ ...newOrder, ekspedisi: e.target.value })
                    }
                    className="w-full px-3 py-1.5 text-xs border border-slate-200 rounded-xl"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewOrder({ ...newOrder, resiNumber: e.target.value })
                    }
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                  Pilih Produk Fashion yang Terjual
                </label>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 text-xs">
                <div>
                  ...
                  <p className="font-bold text-slate-900">
                    Rp {newOrder.grossAmount.toLocaleString("id-ID")}
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-bold text-rose-600">
                    - Rp {newOrder.adminFee.toLocaleString("id-ID")}
                  </p>
                </div>
                <div>
                  <span className="text-[10px] text-slate-500">
                ...
                <div>
                  ...
                  <p className="font-bold text-emerald-700 text-sm">
                    Rp {newOrder.netPayout.toLocaleString("id-ID")}
                  </p>
                </div>
              </div>
      ...
      {showImportModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <h3 className="text-base font-bold text-slate-900">
                Import Penjualan dari Excel / CSV
              </h3>
              <button
            ...
            <div className="py-6 text-center">
              ...
              <p className="text-xs text-slate-600 mb-4">
                Pilih file laporan export dari Shopee, TikTok Shop, atau
                Tokopedia.
              </p>
              <input
                type="file"
      ...
      {showReturnModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200">
            <h3 className="text-base font-bold text-slate-900">
              Proses Retur / Pembatalan Pesanan
            </h3>
            <p className="text-xs text-slate-600 mt-1">
              Pesanan{" "}
              <strong className="font-mono text-slate-900">
                {showReturnModal.orderNumber}
              </strong>{" "}
              ({showReturnModal.customerName})
            </p>

            <div className="mt-4 space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 mb-1">
                  Alasan Retur / Komplain Pelanggan:
                </label>
                <input
                  type="text"
                  value={returnReason}
