...
export const UserManagement: React.FC = () => {
  const { currentUser, users, switchUserRole, loginWithEmailPassword, addUser, updateUserPermissions, updateUserSettings } = useApp();
  ...
  const permissionsMatrix = [
    ...
    {
      module: '6. Keuangan (Buku Kas, Mutasi Bank, Laba Rugi P&L, Hutang Supplier)',
      owner: true,
      admin: false,
      gudang: false,
      produksi: false,
  ...
  const handleAddUserSubmit = (e: React.FormEvent) => {
    ...
    addUser({
      ...
      permissions: {
        dashboard: newUserData.role === 'owner' || newUserData.role === 'finance',
        marketplace: newUserData.role === 'owner' || newUserData.role === 'admin' || newUserData.role === 'finance',
        products: newUserData.role !== 'gudang',
        inventory: newUserData.role === 'owner' || newUserData.role === 'gudang' || newUserData.role === 'produksi',
        production: newUserData.role === 'owner' || newUserData.role === 'produksi',
    ...
    alert('User staff baru berhasil ditambahkan beserta settingan awal profil!');
  };
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-xs">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-600 mt-1">
            Konfigurasi profil kerja, preferensi peringatan stok kritis, gudang default, dan kontrol hak akses setiap divisi.
          </p>
        </div>
            ...
            return (
              <div 
                ...
                className={`bg-white rounded-2xl p-5 border transition-all shadow-xs flex flex-col justify-between ${
                  isCurrent ? 'border-indigo-500 ring-2 ring-indigo-500/10' : 'border-slate-200'
                }`}
              >
                <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-md mt-1 ${
                          u.role === 'owner' ? 'bg-amber-100 text-amber-900' :
                          u.role === 'admin' ? 'bg-blue-100 text-blue-800' :
                          u.role === 'gudang' ? 'bg-purple-100 text-purple-800' :
                          u.role === 'produksi' ? 'bg-orange-100 text-orange-800' : 'bg-emerald-100 text-emerald-800'
                        }`}>
                          {u.role.toUpperCase()}
                        </span>
                      </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 text-xs space-y-1.5">
                    <div className="flex items-center justify-between text-slate-600 text-[11px]">
                      ...
                      <span className="font-semibold text-slate-800">{u.settings?.defaultWarehouse || 'Gudang Utama'}</span>
                    </div>
                    <div className="flex items-center justify-between text-slate-600 text-[11px]">
                      <span>Notifikasi Stok Kritis:</span>
                      <span className={`font-semibold ${u.settings?.enableStockAlerts ? 'text-emerald-700' : 'text-slate-400'}`}>
                        {u.settings?.enableStockAlerts ? 'Aktif (On)' : 'Nonaktif'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-slate-600 text-[11px]">
                      <span>Safety Buffer Stok:</span>
                      <span className="font-semibold text-indigo-700 font-mono">+{u.settings?.minStockBufferPercent || 25}%</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const email = window.prompt(`Masukkan email untuk ${u.name}:`, u.email);
                      if (!email) return;

                      const password = window.prompt(`Masukkan password untuk ${u.email}:`);
                      if (!password) return;

                      const loggedInUser = loginWithEmailPassword(email, password);

                      if (!loggedInUser) {
                        alert('Email atau password salah.\nCoba pakai data berikut:\nowner@sabhirafashion.id / owner123\nfinance@sabhirafashion.id / finance123\nadmin@sabhirafashion.id / admin123\ngudang@sabhirafashion.id / gudang123\nproduksi@sabhirafashion.id / produksi123\nmarketing@sabhirafashion.id / marketing123');
                        return;
                      }

                      alert(`Login berhasil untuk ${loggedInUser.name}`);
                    }}
      ...
      {activeTab === 'settings' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
          <div className="p-4 border-b border-slate-100">
            <h3 className="text-sm font-bold text-slate-900">Settingan Awal Konfigurasi Profil Akun Staff</h3>
            <p className="text-xs text-slate-500">
              Setiap user akun memiliki preferensi tersendiri untuk lokasi gudang prioritas, notifikasi peringatan stok, dan ambang batas safety stock.
            </p>
          </div>
                ...
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-slate-50/70">
                    <td className="py-3 px-4">
                      ...
                      <p className="text-[10px] text-slate-400 font-mono uppercase">{u.role} • {u.email}</p>
                    </td>
      ...
      {activeTab === 'permissions' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
          <div className="p-4 border-b border-slate-100">
            <h3 className="text-sm font-bold text-slate-900">Matriks Hak Akses Modul ERP (Role-Based Access Control)</h3>
            <p className="text-xs text-slate-500">Standar keamanan data Sabhira ERP V2 untuk menjaga kerahasiaan margin laba dan saldo kas.</p>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {permissionsMatrix.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/70">
                    ...
                    <td className="py-3 px-3 text-center">
                      {item.owner ? <Check className="w-4 h-4 text-emerald-600 mx-auto" /> : <X className="w-4 h-4 text-slate-300 mx-auto" />}
                    </td>
                    <td className="py-3 px-3 text-center">
                      {item.admin ? <Check className="w-4 h-4 text-emerald-600 mx-auto" /> : <X className="w-4 h-4 text-slate-300 mx-auto" />}
                    </td>
                    <td className="py-3 px-3 text-center">
                      {item.gudang ? <Check className="w-4 h-4 text-emerald-600 mx-auto" /> : <X className="w-4 h-4 text-slate-300 mx-auto" />}
                    </td>
                    <td className="py-3 px-3 text-center">
                      {item.produksi ? <Check className="w-4 h-4 text-emerald-600 mx-auto" /> : <X className="w-4 h-4 text-slate-300 mx-auto" />}
                    </td>
                    <td className="py-3 px-3 text-center">
                      {item.finance ? <Check className="w-4 h-4 text-emerald-600 mx-auto" /> : <X className="w-4 h-4 text-slate-300 mx-auto" />}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              ...
              <button onClick={() => setShowEditSettingsModal(false)} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>

            <form onSubmit={handleSaveSettings} className="mt-4 space-y-3.5 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Lokasi Gudang Prioritas</label>
                <select
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    onChange={e => setSettingsForm({ ...settingsForm, minStockBufferPercent: Number(e.target.value) })}
                    className="w-24 px-3 py-1.5 text-xs font-bold border border-slate-200 rounded-xl"
                  />
                  <span className="text-slate-500 text-[11px]">
                    Buffer pengingat sebelum stok benar-benar menyentuh batas kritis.
                  </span>
                </div>
              </div>

              <div className="space-y-2 pt-2 border-t border-slate-100">
                <p className="font-bold text-slate-800 text-xs">Preferensi Peringatan & Notifikasi:</p>
                
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={settingsForm.enableStockAlerts}
                    onChange={e => setSettingsForm({ ...settingsForm, enableStockAlerts: e.target.checked })}
                    className="rounded text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="text-slate-700 font-medium">Peringatan Otomatis Stok Kritis / Habis</span>
                </label>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={e => setSettingsForm({ ...settingsForm, enableEscrowAlerts: e.target.checked })}
                    className="rounded text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="text-slate-700 font-medium">Peringatan Pencairan Dana Marketplace (Escrow)</span>
                </label>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={e => setSettingsForm({ ...settingsForm, enableProductionAlerts: e.target.checked })}
                    className="rounded text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="text-slate-700 font-medium">Peringatan Deadline SPK & Pengurangan Bahan</span>
                </label>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={e => setSettingsForm({ ...settingsForm, notificationSound: e.target.checked })}
                    className="rounded text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="text-slate-700 font-medium">Suara Efek Alarm Peringatan (Sound Alert)</span>
                </label>
              </div>
      ...
      {showAddUserModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <h3 className="text-base font-bold text-slate-900">Tambah Akun Staff Baru</h3>
            ...
            <form onSubmit={handleAddUserSubmit} className="mt-4 space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Nama Lengkap Staff</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Role / Jabatan & Hak Akses</label>
                <select
                  value={newUserData.role}
                  onChange={e => setNewUserData({ ...newUserData, role: e.target.value as any })}
                  className="w-full px-3 py-2 text-xs border border-slate-200 rounded-xl font-medium"
                >
                  <option value="admin">Admin Marketplace (Shopee, TikTok, Resi)</option>
                  <option value="gudang">Tim Gudang (Barang Masuk/Keluar, Opname)</option>
                  <option value="produksi">Tim Produksi (SPK, Bahan Baku, QC)</option>
                  <option value="finance">Tim Keuangan (Buku Kas, Laba Rugi, Payout)</option>
